...
var initGame = regexp.MustCompile("^InitGame: (?P<vars>.*)$")
var exitLevel = regexp.MustCompile("^ExitLevel: executed$")
var shutdownGame = regexp.MustCompile("^ShutdownGame:$")
var roundSeperator = regexp.MustCompile("^-+")
...
func Parse(line string) interface{} {
	...
	// TODO create regex for everything
	switch {
	case initGame.MatchString(line):
		ret := InitGame{Unix: time.Now().Unix(), Vars: nil}
		return ret
	case exitLevel.MatchString(line):
		ret := ExitLevel{Unix: time.Now().Unix()}
		return ret
	case shutdownGame.MatchString(line):
		ret := ShutdownGame{Unix: time.Now().Unix()}
		return ret
  case roundSeperator.MatchString(line):
    return nil
	default:
		switch values[0] {
		case "J":
